...
_BASE_PATH = Path(__file__).with_name("seminar11_oop_immersive_v9_overlay.py")
_SPEC = importlib.util.spec_from_file_location("oop_v9_base", _BASE_PATH)
...
class Seminar11OOPImmersiveV10(_BASE.Seminar11OOPImmersiveV9):
    """Final visual-QA correction layer: clean behavior label and message routing."""
    ...
    def real_object_to_model(self):
        ...
        q2 = self.text("WHAT CAN IT DO?", 22, BOLD, MID_GRAY).move_to(RIGHT * 4.45 + DOWN * 1.72)
        ...
        self.play(
            Transform(bot, bot_target), Transform(energy, energy_new), Transform(position, position_new),
            Indicate(move_chip, color=BLACK), run_time=RUN_S,
        )
        self.wait(PAUSE_R)
        ...
        boundary = RoundedRectangle(
            ...
            stroke_color=BLACK, stroke_width=2.5, fill_color=PAPER, fill_opacity=0.40,
    ...
    def composition(self):
        ...
        floor = RoundedRectangle(
            ...
            stroke_color=BLACK, stroke_width=2.2, fill_color=PAPER, fill_opacity=0.55,
        ).move_to(DOWN * 0.45)
        ...
        msg1 = self.text("reserve()", 17, BOLD, MID_GRAY).next_to(route1, UP, buff=0.12)
        ...
        msg2 = self.text("deliver()", 17, BOLD, MID_GRAY).next_to(route2, UP, buff=0.12)
