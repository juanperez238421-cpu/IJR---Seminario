...
const allowedOperations: Record<string, Route> = {
  ...
  seminar_master_dashboard_v2: { rpc: "seminar_master_dashboard_v2", args: [], admin: true, teacherTokenArg: false },
  ...
  seminar_project_decision: { rpc: "seminar_student_project_decide", args: ["p_student_registry_id","p_decision_status","p_decision_note"], admin: true, teacherTokenArg: false },
...
function cors(origin: string | null) { return {"Access-Control-Allow-Origin": origin && allowedOrigins.has(origin) ? origin : "null","Access-Control-Allow-Headers":"authorization, apikey, content-type, x-client-info","Access-Control-Allow-Methods":"POST, OPTIONS","Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store","Vary":"Origin"}; }
...
Deno.serve(async (req: Request) => {
  ...
  const url=Deno.env.get("SUPABASE_URL")??"",anonKey=Deno.env.get("SUPABASE_ANON_KEY")??"",serviceRoleKey=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")??"",authorization=req.headers.get("Authorization")??"",token=authorization.replace(/^Bearer\s+/i,""),requestId=req.headers.get("sb-request-id")??crypto.randomUUID(),userAgent=(req.headers.get("User-Agent")??"").slice(0,1000),ipHash=await sha256((req.headers.get("x-forwarded-for")??"").split(",")[0].trim()),claims=parseJwtPayload(token);
  const userClient=createClient(url,anonKey,{global:{headers:{Authorization:authorization}},auth:{persistSession:false,autoRefreshToken:false}}),adminClient=createClient(url,serviceRoleKey,{auth:{persistSession:false,autoRefreshToken:false}});
  ...
  try{
    const payload=await req.json(); operation=typeof payload?.operation==="string"?payload.operation:"unknown"; const route=allowedOperations[operation];
    if(!route){await audit("deny","operation_not_allowed");return json(origin,404,{error:"not_found"});}
    const{data:userData,error:userError}=await userClient.auth.getUser(token); if(userError||!userData.user){await audit("deny","invalid_user_session");return json(origin,404,{error:"not_found"});} userId=userData.user.id;
    const{data:aal,error:aalError}=await userClient.auth.mfa.getAuthenticatorAssuranceLevel(token); if(aalError||aal?.currentLevel!=="aal2"){await audit("deny","aal2_required");return json(origin,403,{error:"mfa_required"});}
    const{data:profile,error:profileError}=await adminClient.from("profiles").select("role,active").eq("auth_user_id",userId).eq("active",true).in("role",["teacher","admin"]).maybeSingle(); if(profileError||!profile){await audit("deny","teacher_role_required");return json(origin,404,{error:"not_found"});}
    const supplied=payload?.args&&typeof payload.args==="object"?payload.args:{},rpcArgs:Record<string,unknown>={}; if(route.teacherTokenArg!==false)rpcArgs.p_teacher_token=""; for(const key of route.args)rpcArgs[key]=supplied[key]??null;
    const rpcClient=route.admin?adminClient:userClient; const{data,error}=await rpcClient.rpc(route.rpc,rpcArgs); if(error){await audit("error",`${route.rpc}:${error.code??"rpc_error"}`);return json(origin,400,{error:"operation_failed"});}
    await audit("allow",route.rpc); return json(origin,200,{data,request_id:requestId});
  }catch(error){await audit("error",error instanceof Error?error.message:"unexpected_error");return json(origin,400,{error:"invalid_request"});}
});
