...
test('OOP + UML registration contains exactly one student credential field: institutional email', async()=>{
  ...
  assert.match(html,/data-oop-uml-build="20260923-email-v12"/);
  assert.match(html,/hub\.js\?v=20260923-email-v12/);
});
...
test('Supabase static host rewrites any legacy OOP + UML registration server-side', async()=>{
  ...
  assert.match(edge,/OOP_UML_EMAIL_BUILD = "20260923-email-v12"/);
