...
V2.1 visual QA revision: larger classroom typography, cleaner transitions, no overlap zones.
...
Revision marker: V2.1 · 2026-09-23.
"""
...
class Seminar11Class03ConstructorsValidState(JPClassroomScene):
    ...
    def object_card(
        ...
        *,
        ...
        title = self.text(f"{variable} : Robot", 24 if not compact else 21, BOLD)
        divider = Line(LEFT * (width / 2 - 0.22), RIGHT * (width / 2 - 0.22), color=LIGHT_GRAY, stroke_width=1.4)
        rows = VGroup(
            self.state_row("name =", f'"{name}"' if name != "?" else "?", width=width - 0.55, size=22 if not compact else 19),
            self.state_row("energy =", str(energy), width=width - 0.55, size=22 if not compact else 19),
            self.state_row("position =", str(position), width=width - 0.55, size=22 if not compact else 19),
    ...
    def measurement_card(self, *, width: float = 3.75, height: float = 2.05) -> VGroup:
        ...
        title = self.text("m : Medicion", 24, BOLD)
        divider = Line(LEFT * (width / 2 - 0.22), RIGHT * (width / 2 - 0.22), color=LIGHT_GRAY, stroke_width=1.4)
        rows = VGroup(
            self.state_row("valor =", "12", width=width - 0.55, size=22),
            self.state_row("unidad =", '"C"', width=width - 0.55, size=22),
    ...
    def opening_00(self) -> None:
        ...
        self.play(FadeOut(callback), run_time=RUN_QUICK)
        self.play(FadeIn(question, shift=UP * 0.08), FadeIn(but), run_time=RUN_SLOW)
        self.wait(PAUSE_EXPLAIN)
    ...
    def scene_01_recall_atlas(self) -> None:
        ...
        robot = self.robot_icon("Atlas", 90, scale=1.30).move_to(LEFT * 4.55 + DOWN * 0.02)
        atlas_label = self.text("Atlas", 28, BOLD).next_to(robot, DOWN, buff=0.30)
        card = self.object_card("atlas", "Atlas", 90, 0, width=4.45, height=3.05).move_to(RIGHT * 2.65 + UP * 0.25)
    ...
    def scene_02_creation_problem(self) -> None:
        ...
        call = self.mono("atlas = Robot()", 34, BOLD).move_to(LEFT * 3.85 + UP * 1.65)
        robot = self.robot_icon("", None, scale=1.00, incomplete=True).move_to(LEFT * 4.20 + DOWN * 0.45)
        empty = self.object_card("atlas", "?", "?", "?", width=3.95, height=2.75).move_to(RIGHT * 3.45 + UP * 0.55)
        ...
        problem = VGroup(
            self.text("Between creation and the last assignment,", 30),
            self.text("the object is incomplete.", 33, BOLD),
    ...
    def scene_03_constructor_model(self) -> None:
        ...
        inputs_title = self.text("INPUT DATA", 25, BOLD).move_to(LEFT * 5.15 + UP * 1.55)
        ...
        gate_text = VGroup(self.text("CONSTRUCTOR", 30, BOLD), self.mono("Robot(...)", 26)).arrange(DOWN, buff=0.22).move_to(gate)
        gate_group = VGroup(gate, gate_text)
        card = self.object_card("atlas", "Atlas", 90, 0, width=3.95, height=2.75).move_to(RIGHT * 4.55 + DOWN * 0.05)
        ...
        statement = self.text("OBJECT CREATION IS A PROCESS.", 34, BOLD).move_to(UP * 2.10)
    ...
    def scene_04_python_init(self) -> None:
        ...
        panel, code = self.code_panel(lines, width=8.60, height=4.35, font_size=27, title="PYTHON")
        ...
        params_tag = VGroup(
            self.text("name · energy · position", 26, BOLD),
            self.text("required creation information", 22),
        ...
        left = VGroup(self.mono("name", 25, BOLD), self.text("PARAMETER", 18)).arrange(DOWN, buff=0.10).move_to(mapping_box.get_center() + LEFT * 1.20)
        right = VGroup(self.mono("self.name", 25, BOLD), self.text("OBJECT ATTRIBUTE", 18)).arrange(DOWN, buff=0.10).move_to(mapping_box.get_center() + RIGHT * 1.10)
    ...
    def scene_05_self(self) -> None:
        ...
        calls = VGroup(
            self.mono('atlas = Robot("Atlas", 90, 0)', 26, BOLD),
            self.mono('explorer = Robot("Explorer", 75, 4)', 26),
        ...
        atlas = self.object_card("atlas", "Atlas", 90, 0, width=3.70, height=2.30, compact=True).move_to(LEFT * 3.55 + DOWN * 1.45)
        explorer = self.object_card("explorer", "Explorer", 75, 4, width=3.70, height=2.30, compact=True).move_to(RIGHT * 3.55 + DOWN * 1.45)
    ...
    def scene_06_three_objects(self) -> None:
        ...
        cards = VGroup(*[
            self.object_card(r.variable, r.name, r.energy, r.position, width=3.85, height=2.35, compact=True)
            for r in ROBOTS
        ]).arrange(RIGHT, buff=0.40).move_to(UP * 0.02)
        calls = VGroup(*[
            self.mono(f'{r.variable} = Robot("{r.name}", {r.energy}, {r.position})', 20)
            for r in ROBOTS
        ]).arrange(DOWN, aligned_edge=LEFT, buff=0.14).move_to(DOWN * 2.45)
    ...
    def scene_07_valid_state(self) -> None:
        ...
        panel, code = self.code_panel(lines, width=8.40, height=4.95, font_size=22, title="EARLY VALIDATION")
    ...
    def scene_08_constructor_vs_method(self) -> None:
        ...
        ltitle = self.text("CREATION", 28, BOLD).next_to(left.get_top(), DOWN, buff=0.28)
        rtitle = self.text("BEHAVIOR", 28, BOLD).next_to(right.get_top(), DOWN, buff=0.28)
        lflow = VGroup(
            self.mono('Robot("Atlas", 90, 0)', 24), self.text("↓", 28), self.mono("__init__()", 25, BOLD), self.text("↓", 28), self.tag("atlas exists", 2.15, 0.58, 20, filled=True)
        ).arrange(DOWN, buff=0.14).move_to(left.get_center() + DOWN * 0.15)
        before = self.state_row("position =", "0", width=2.6, size=21)
        call = self.mono("atlas.move(3)", 24, BOLD)
    ...
    def scene_09_uml_sync(self) -> None:
        ...
        code_line = self.mono("def __init__(self, name, energy, position):", 21, BOLD).move_to(code_box)
        uml_sig = self.mono("+ Robot(name, energy, position)", 23, BOLD).move_to(RIGHT * 4.20 + UP * 1.55)
    ...
    def scene_10_medicion_transfer(self) -> None:
        ...
        problem = VGroup(
            self.text("NEW PROBLEM", 25, BOLD),
            self.text("A measurement has:", 29),
            self.text("• a numeric value", 28),
            self.text("• a unit", 28),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.17).move_to(LEFT * 4.55 + UP * 0.55)
        question = self.text("What does a Medicion need to exist?", 29, BOLD).move_to(LEFT * 4.05 + DOWN * 1.35)
        reveal = VGroup(self.tag("valor", 1.75, 0.62, 22), self.tag("unidad", 1.90, 0.62, 22)).arrange(RIGHT, buff=0.32).next_to(question, DOWN, buff=0.30)
        self.play(FadeIn(problem), run_time=RUN_NORMAL)
        self.play(FadeIn(question), run_time=RUN_NORMAL)
        self.wait(PAUSE_EXPLAIN)
        ...
        # V2.1: split the code into two readable blocks. Blank Text lines in the
        # previous version collapsed during layout and produced severe overprinting.
        constructor_lines = [
            "class Medicion:",
        ...
        constructor_panel, ccode = self.code_panel(constructor_lines, width=8.15, height=2.75, font_size=23, title="m09 · PYTHON · CONSTRUCTOR")
        method_panel, mcode = self.code_panel(method_lines, width=8.15, height=2.45, font_size=23, title="METHOD + USE")
        constructor_panel.move_to(LEFT * 3.55 + UP * 0.65)
        method_panel.move_to(LEFT * 3.55 + DOWN * 2.10)
        left_code = VGroup(constructor_panel, method_panel)

        card = self.measurement_card(width=4.05, height=2.20).move_to(RIGHT * 4.35 + UP * 0.78)
        self.play(FadeIn(constructor_panel), FadeIn(method_panel), run_time=RUN_NORMAL)
        self.play(Circumscribe(ccode[2], color=BLACK_LINE), Circumscribe(ccode[3], color=BLACK_LINE), run_time=RUN_NORMAL)
        self.play(FadeIn(card, shift=RIGHT * 0.10), run_time=RUN_NORMAL)
        self.wait(PAUSE_READ)

        stored_box = RoundedRectangle(width=2.55, height=1.25, corner_radius=0.10, stroke_color=BLACK_LINE, stroke_width=1.5, fill_color=VERY_LIGHT_GRAY, fill_opacity=1)
        stored_txt = VGroup(self.mono("self.valor", 24, BOLD), self.text("STORED STATE", 17, BOLD)).arrange(DOWN, buff=0.07).move_to(stored_box)
        stored = VGroup(stored_box, stored_txt)
        temp_box = stored_box.copy()
        temp_txt = VGroup(self.mono("factor", 24, BOLD), self.text("METHOD PARAMETER", 16, BOLD)).arrange(DOWN, buff=0.07).move_to(temp_box)
        temp = VGroup(temp_box, temp_txt)
        compare = VGroup(stored, temp).arrange(RIGHT, buff=0.28).move_to(RIGHT * 4.35 + DOWN * 1.18)
        calc = self.mono("12 × 2 = 24", 31, BOLD).move_to(RIGHT * 4.35 + DOWN * 2.55)
        self.play(FadeIn(compare), run_time=RUN_NORMAL)
        self.play(Circumscribe(mcode[1], color=BLACK_LINE), run_time=RUN_NORMAL)
        self.play(FadeIn(calc), run_time=RUN_NORMAL)
        self.wait(PAUSE_EXPLAIN)

        # Clear every explanatory element before the transfer question.
        self.safe_fade(left_code, card, compare, calc)
        transfer_q = VGroup(
            self.text("WHY DOES self.valor REMAIN AFTER __init__ FINISHES,", 27, BOLD),
            self.text("BUT factor DOES NOT?", 29, BOLD),
        ).arrange(DOWN, buff=0.12).move_to(UP * 0.25)
        self.fit(transfer_q, 13.6, 1.45)
        self.play(FadeIn(transfer_q), run_time=RUN_NORMAL)
        self.wait(PAUSE_WORK)
        answer = VGroup(
            self.text("self.valor belongs to the object.", 28, BOLD),
            self.text("factor belongs to one method call.", 28, BOLD),
        ).arrange(DOWN, buff=0.15).move_to(UP * 0.25)
        self.play(FadeOut(transfer_q), run_time=RUN_QUICK)
        self.play(FadeIn(answer), run_time=RUN_NORMAL)
        self.wait(PAUSE_EXPLAIN)
        self.clear_scene()
    ...
    def scene_11_common_mistake(self) -> None:
        ...
        left_lines = VGroup(
            self.mono("Robot()", 27, BOLD),
            self.text("↓", 27),
            self.text("scattered initialization", 23),
            self.text("↓", 27),
            self.text("repeated logic", 23),
            self.text("↓", 27),
            self.text("possible invalid state", 23, BOLD),
        ).arrange(DOWN, buff=0.12).move_to(left.get_center() + DOWN * 0.15)
        right_lines = VGroup(
            self.mono('Robot("Atlas", 90, 0)', 25, BOLD),
            self.text("↓", 27),
            self.text("required data", 23),
            self.text("↓", 27),
            self.text("constructor + validation", 23),
            self.text("↓", 27),
            self.text("ready object", 23, BOLD),
    ...
    def final_synthesis(self) -> None:
        ...
        evidence = VGroup(
            self.text("1 · THREE valid objects", 26),
            self.text("2 · Justify each constructor parameter", 26),
            self.text("3 · UML consistent with implementation", 26),
            self.text("4 · One rejected invalid initial state", 26),
            self.text("5 · Explain self.valor vs factor", 26),
