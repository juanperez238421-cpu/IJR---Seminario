...
class Seminar11Class03ConstructorsValidState(Scene):
    ...
    # ------------------------------------------------------------------
    # CHAPTER 10 — transfer to repository m09
    # ------------------------------------------------------------------
    def chapter_10_m09_transfer(self):
        self.set_header(10, "M09 · CONSTRUCTORS, ATTRIBUTES & METHODS", "Transfer the same mental model to the real technical module: Medicion.")

        uml = self.make_uml_class(
            "Medicion", ["valor", "unidad"], ["Medicion(valor, unidad)", "escalar(factor)"],
            width=4.05, height=3.50, font_size=21,
        ).move_to(LEFT * 5.15 + DOWN * 0.25)
        code, cl = self.make_code_panel([
            "class Medicion:",
            ...
            'm = Medicion(12, "C")',
            "print(m.escalar(2))",
        ], width=8.15, font_size=20, title="M09 · PYTHON SOURCE")
        code.move_to(RIGHT * 2.95 + DOWN * 0.15)
        self.play(FadeIn(uml), FadeIn(code), run_time=1.0)
        self.wait(3.0)

        flow1 = VGroup(
            self.make_argument_token("12", 1.15), self.t("→", 24, BOLD), self.code_t("valor", 22), self.t("→", 24, BOLD), self.code_t("self.valor", 22, BOLD)
        ).arrange(RIGHT, buff=0.18).move_to(DOWN * 2.55 + LEFT * 1.1)
        flow2 = VGroup(
            self.make_argument_token('"C"', 1.15), self.t("→", 24, BOLD), self.code_t("unidad", 22), self.t("→", 24, BOLD), self.code_t("self.unidad", 22, BOLD)
        ).arrange(RIGHT, buff=0.18).next_to(flow1, DOWN, buff=0.32)
        self.play(FadeIn(flow1), FadeIn(flow2), run_time=0.9)
        self.wait(3.0)

        self.play(FadeOut(flow1), FadeOut(flow2), run_time=0.6)
        factor = self.make_argument_token("factor = 2", 2.0).move_to(LEFT * 0.2 + DOWN * 2.75)
        method = self.label_box("m.escalar(2)", 2.45, 0.60, 20, True).move_to(RIGHT * 2.6 + DOWN * 2.75)
        temp_note = self.t("temporary during this method call", 20, color=DARK_GRAY).next_to(factor, DOWN, buff=0.15)
        self.play(FadeIn(factor), FadeIn(method), FadeIn(temp_note), run_time=0.8)
        self.wait(2.4)
        self.play(factor.animate.move_to(method), run_time=0.85)
        self.play(FadeOut(factor), run_time=0.55)
        qa = VGroup(
            self.t("WHAT STAYS IN THE OBJECT?", 21, BOLD),
            self.code_t("self.valor   self.unidad", 22, BOLD),
            self.t("WHAT EXISTS ONLY DURING THE CALL?", 21, BOLD),
            self.code_t("factor", 22, BOLD),
        ).arrange(DOWN, buff=0.18).move_to(UP * 2.15)
        self.play(FadeIn(qa), run_time=0.8)
        self.wait(4.2)
        self.clear_content_keep_header()
